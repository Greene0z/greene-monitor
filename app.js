...
let S = {
  ...
  profile:null, gameProfile:null, notesPage:0, notesHasMore:false,
...
async function insertLocal(table,payload){
  ...
  if(error){toast(error.message);return null;} const key=stateKey[table]; if(key)mergeRows(key,[data]); toast("Salvo"); return data;
}
async function upsertDateLocal(table,payload){
  ...
  if(error){toast(error.message);return null;} const key=stateKey[table]; if(key)mergeRows(key,[data]); render(); toast("Salvo"); return data;
}
async function deleteLocal(table,id,ask=true){
  ...
  const key=stateKey[table]; if(key)S[key]=S[key].filter(x=>x.id!==id); render(); return true;
}
...
async function newNote(templateId=null){
  ...
  const i=S.notes.findIndex(n=>n.id===tempId);if(i>=0)S.notes[i]={...data,content:undefined};delete S.noteContent[tempId];S.noteContent[data.id]=data.content||"";S.selectedNote=data.id;render();setTimeout(()=>document.querySelector('form[data-form="note"] input[name="title"]')?.select(),0);
}
async function openDailyNote(date=today()){
  ...
  S.selectedNote=n.id;S.view="notes";await loadNoteContent(n.id);render();
}
...
function dashboardCards(){
  ...
  const map={
    ...
    rpg:()=>S.profile?.gamification_enabled?`<div class="panel dash-card hoverable"><div class="rpg-preview"><div class="pixel-avatar rpg-mini ${esc(g.presentation||"neutral")}"><i class="hair"></i><i class="head"></i><i class="eye1"></i><i class="eye2"></i><i class="body"></i></div><div><small>Personagem · prévia V2.3</small><div class="metric">Lv. ${g.level||1}</div><div class="submetric"><span>${esc(g.character_name||"Aventureiro")}</span><span>·</span><span>${g.xp||0} XP</span></div></div></div></div>`:""
  };
...
function settingsPage(){
  ...
  <div><div class="panel setting-section"><h3>Personagem — preparação V2.3</h3><div class="avatar-settings"><div class="pixel-avatar ${esc(g.presentation||"neutral")}"><i class="hair"></i><i class="head"></i><i class="eye1"></i><i class="eye2"></i><i class="body"></i></div><form data-form="game-profile"><div class="field"><label>Nome do personagem</label><input name="character_name" maxlength="40" value="${esc(g.character_name||p.display_name||"Aventureiro")}" required></div><div class="field"><label>Apresentação do avatar</label><select name="presentation"><option value="feminine" ${g.presentation==="feminine"?"selected":""}>Feminina</option><option value="masculine" ${g.presentation==="masculine"?"selected":""}>Masculina</option><option value="neutral" ${g.presentation==="neutral"?"selected":""}>Neutra</option></select></div><div class="actions"><button>Salvar personagem</button></div></form></div><p class="muted">A V2.3 usará essa preferência no criador pixel art. A prévia atual é apenas estrutural.</p></div>
...
function headerConfig(){
  return {today:["＋ Registrar","registerTop"],calendar:null,habits:["＋ Novo hábito","habitCreate"],notes:["＋ Nova nota","newNote"],books:["＋ Novo livro","bookCreate"],studies:["＋ Registrar estudo","studyCreate"],goals:["＋ Novo objetivo","goalCreate"],analysis:null,settings:null}[S.view];
}
function render(){
  const names={today:S.selected===today()?"Hoje":toDate(S.selected).toLocaleDateString("pt-BR",{day:"numeric",month:"long"}),calendar:"Calendário",habits:"Hábitos",notes:"Notas",books:"Leituras",studies:"Estudos",goals:"Objetivos",analysis:"Análises",settings:"Configurações"};
  ...
  const pages={today:todayPage,calendar:calendarPage,habits:habitsPage,notes:notesPage,books:booksPage,studies:studiesPage,goals:goalsPage,analysis:analysisPage,settings:settingsPage};
...
function renderCommands(q){
  q=(q||"").trim().toLowerCase();const actions=[["Nova nota","Criar nota","notes","newnote"],["Nota diária","Abrir/criar hoje","notes","daily"],["Registrar leitura","Abrir Hoje","today",""],["Registrar estudo","Abrir Estudos","studies",""],["Marcar hábito","Abrir Hábitos","habits",""],["Abrir calendário","Calendário","calendar",""],["Configurações","Personalizar","settings",""]];
...
document.addEventListener("submit", async (e)=>{
  ...
  if(t==="profile"){const payload={app_name:x.app_name.trim(),display_name:x.display_name.trim()||null,accent:x.accent,gamification_enabled:x.gamification_enabled==="true",updated_at:new Date().toISOString()};const {data,error}=await sb.from("profiles").update(payload).eq("user_id",S.user.id).select().single();if(error)return toast(error.message);S.profile=data;applyProfile();render();toast("Personalização salva");return}
  if(t==="game-profile"){const payload={character_name:x.character_name.trim(),presentation:x.presentation,updated_at:new Date().toISOString()};const {data,error}=await sb.from("game_profiles").update(payload).eq("user_id",S.user.id).select().single();if(error)return toast(error.message);S.gameProfile=data;render();toast("Personagem salvo");return}
